...
- *(infisical)* Add connection, binding, and provenance data model
- *(infisical)* Add universal-auth API client
- *(infisical)* Add environment secret sync action
- *(infisical)* Schedule periodic binding sync
- *(infisical)* Add inherited secret resolver
- *(infisical)* Inject inherited secrets into deployments
- *(infisical)* Add connection and binding authorization policies
- *(infisical)* Add connection settings UI and inherited variable badges
- *(infisical)* Add a settings menu entry, binding lifecycle, and bounded sync errors
- *(infisical)* Derive folder paths from Coolify project and resource names
- *(infisical)* Add environment, folder and batch-upsert client operations
- *(infisical)* Enforce the managed-variable lock in Eloquent hooks
- *(infisical)* Mark Coolify's own variable writes as system writes
- *(infisical)* Push existing Coolify variables up on first adoption
- *(infisical)* Pull continuously and freshly at deploy
- *(infisical)* Push Coolify-generated secrets up as they are created
- *(infisical)* Lock and sync managed database credential columns
- *(infisical)* Lock the human variable surfaces
- *(infisical)* Rebuild the settings screen around one connection
- *(infisical)* Add a direct sidebar link to the Infisical screen
- *(infisical)* Add a Sync now action that also recovers a drained project
- *(notifications)* Add Microsoft Teams channel
...
- *(infisical)* Add missing return types on relation and scope methods
- *(infisical)* Wrap connection failures and reject hidden secret values
- *(infisical)* Shadow user-owned key collisions, transaction-wrap sync mutations
- *(infisical)* Escape inherited values and rank them below coolify variables
- *(infisical)* Make binding-owned shared variables read-only on the environment screen
- *(infisical)* Stop claiming inherited secrets on databases
- *(infisical)* Refuse to wipe every synced row on an empty upstream response
- *(infisical)* Stop leaking decrypted secrets into the edit form's wire:snapshot
- *(infisical)* Close whitespace-only credential wipe on edit and create
- *(infisical)* Arm the lock only after adoption succeeds, and report failures
- *(infisical)* Repoint the client at endpoints verified on self-hosted
- *(infisical)* Stop mounting the connection form twice on one page
...
- *(infisical)* Remove the per-environment binding layer
- *(infisical)* Move the sidebar link into the Manage section
...
- Design for Infisical secret sync and container registry credentials
- *(infisical)* Spec single-connection design with continuous sync
- *(infisical)* Record verified Infisical API surface
- *(infisical)* Move the lock into Eloquent hooks and scope DB credentials in
- *(infisical)* Map Coolify's four shared-variable scopes onto Infisical
- *(infisical)* Spec instance-wide connection with a project per team
- *(infisical)* Record verified project-creation API and its traps
...
- *(infisical)* Pin the cross-team escalation shape the brief's guess missed
...
- Build and publish the production image to this fork's GHCR
- Run Pint and the test suite on pull requests
